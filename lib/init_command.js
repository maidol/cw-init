...
module.exports = class Command {
  constructor(options) {
    options = options || {};
    this.name = options.name || 'cw-init';
    this.configName = options.configName || 'cw-init-config';
  ...
  * run(cwd, args) {
  ...
  getParser() {
    return yargs
      .usage('init cw project from boilerplate.\nUsage: $0 [dir] --type=simple')
  ...
  * getTemplateDir() {
    let templateDir;
    // when use `cw-init --template=PATH`
  ...
  /**
   * fetch boilerplate mapping from `cw-init-config`
   * @param {String} [pkgName] - config package name, default to `this.configName`
   * @return {Object} boilerplate config mapping, `{ "cw-egg": { "name": "cw-egg", "package": "cw-egg-boilerplate-simple", "description": "Simple cw-egg app boilerplate" } }`
   */
  ...
  * getPackageInfo(pkgName, withFallback) {
   ...
   *
   * @param {Object} mapping - boilerplate config mapping, `{ "cw-egg": { "name": "cw-egg", "package": "cw-egg-boilerplate-simple", "description": "Simple cw-egg app boilerplate" } }`
  ...
  * downloadBoilerplate(pkgName) {
    ...
    const saveDir = path.join(os.tmpdir(), 'cw-init-boilerplate');
  ...
  * askForVariable(targetDir, templateDir) {
    ...
    try {
      ...
      if (questions.name && !questions.name.default) {
        questions.name.default = path.basename(targetDir).replace(/^cw-/, '');
      }
    } catch (err) {
